  ...
  const fetchPayments = async () => {
    try {
      ...
      const enhancedStats = {
        ...
        paidCount: response.payments.filter((p: ArtistPayment) => p.status === 'paid').length,
        pendingCount: response.payments.filter((p: ArtistPayment) => p.status === 'pending').length,
        failedCount: response.payments.filter((p: ArtistPayment) => p.status === 'failed').length,
      };
                ...
                <tr key={payment._id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className="flex-shrink-0 h-10 w-10">
                        {(payment as any).artistBookingId?.bookedArtist?.roleProfile?.profileImage || (payment as any).artistBookingId?.bookedArtist?.profilePicture ? (
                          <img
                            className="h-10 w-10 rounded-full object-cover"
                            src={(payment as any).artistBookingId.bookedArtist.roleProfile?.profileImage || (payment as any).artistBookingId.bookedArtist.profilePicture}
                            alt={(payment as any).artistBookingId.bookedArtist.roleProfile?.stageName || 'Artist'}
                          />
                        ) : (
                      ...
                      <div className="ml-4">
                        <div className="text-sm font-medium text-gray-900">
                          {(payment as any).artistBookingId?.bookedArtist?.roleProfile?.stageName || 'N/A'}
                        </div>
                        <div className="text-sm text-gray-500">
                          {(payment as any).artistBookingId?.bookedArtist?.firstName} {(payment as any).artistBookingId?.bookedArtist?.lastName}
                        </div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-green-600">
                      {formatCurrency(payment.artistBookingId?.price || payment.totalPrice)}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-gray-900">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-900">
                        {(selectedPayment as any).artistBookingId?.bookedArtist?.roleProfile?.stageName || 'N/A'}
                      </p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Real Name</label>
                      <p className="text-sm text-gray-900">
                        {(selectedPayment as any).artistBookingId?.bookedArtist?.firstName} {(selectedPayment as any).artistBookingId?.bookedArtist?.lastName}
                      </p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Email</label>
                      <p className="text-sm text-gray-900">
                        {(selectedPayment as any).artistBookingId?.bookedArtist?.email || 'N/A'}
                      </p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Artist Type</label>
                      <p className="text-sm text-gray-900">
                        {(selectedPayment as any).artistBookingId?.bookedArtist?.roleProfile?.artistType || 'N/A'}
                      </p>
                    </div>
                  </div>
                </div>
